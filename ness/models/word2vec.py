...
class Word2Vec(BaseModel):
    ...
    def build_model(self, fasta_file:str, epochs=3) -> None:
        ...
        with open(self.temp_corpus_file, 'w') as corpus_writer:
            for s, sequence_ngrams in enumerate(FASTANgramIterator(fasta_file, ksize=self.config['ksize'])):
                corpus_writer.write(sequence_ngrams)

        self.model.build_vocab(corpus_file=self.temp_corpus_file, progress_per=1000)
